...
## Added computeLikelihood method, which avoids the following problems:
##      1 - the mask problem. Compute_log_likelihood was designed for problems with previous transformations, which
##          required a "mask". Given that our experiments didnt apply transformations, this requirement caused an exception
##      2 - the log problem. np.log generates -inf when faced with a likelihood value of 0. This is easily avoided
##          with a simple mask: https://stackoverflow.com/questions/21752989/numpy-efficiently-avoid-0s-when-taking-logmatrix
def compute_log_likelihood_2(
        Xtrue_ND,
        C,
        hidden,
        params,
    ...
    # finally, apply log transform
    for d in xrange(D):
        lik[mask,d] = np.log(lik[mask,d], out=np.zeros_like(lik[mask,d]), where=(lik[mask,d]!=0))
    return lik
